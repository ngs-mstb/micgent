...
def _gff_line_map(line, params):
    ...
    def _split_keyvals(keyval_str):
        ...
        for key, vals in list(quals.items()):
    ...
    if line[:2] == "##":
        ...
        if params.limit_info:
            for limit_name, limit_values in list(params.limit_info.items()):
...
def _gff_line_reduce(map_results, out, params):
    ...
    for key, vals in list(final_items.items()):
        if params.jsonify:
            vals = simplejson.dumps(vals)
        out.add(key, vals)

class _MultiIDRemapper(object):
...
class _AbstractMapReduceGFF(object):
    ...
    def _normalize_limit_info(self, limit_info):
        ...
        final_limit_info = {}
        if limit_info:
            for key, values in list(limit_info.items()):
    ...
    def _add_directives(self, base, directives):
        ...
        for key, vals in list(dir_keyvals.items()):
            for rec in list(base.values()):
    ...
    def _add_parent_child_features(self, base, parents, children):
        ...
        while len(children) > 0:
            parent_id, cur_children = next(itertools.islice(list(children.items()),
                    1))
    ...
    def _identify_dup_ids(self, parents):
        ...
        multi_ids = [(mid, parents) for (mid, parents) in list(multi_ids.items())
                if len(parents) > 1]
        multi_remap = dict()
    ...
    def _add_annotations(self, base, anns):
        ...
        for ann in anns:
            ...
            for key, vals in list(ann['quals'].items()):
                self._add_ann_to_rec(rec, key, vals)
        return base
...
class _GFFParserLocalOut(object):
...
class GFFExaminer(object):
    ...
    def available_limits(self, gff_handle):
        ...
        """
        cur_limits = dict()
        for filter_key in list(self._filter_info.keys()):
        ...
        for line in gff_handle:
            ...
            if line.strip() and line.strip()[0] != "#":
                ...
                assert len(parts) >= 8, line
                parts = parts[:9]
                for filter_key, cur_indexes in list(self._filter_info.items()):
                    cur_id = tuple([parts[i] for i in cur_indexes])
                    cur_limits[filter_key][cur_id] += 1
        # get rid of the default dicts
        final_dict = dict()
        for key, value_dict in list(cur_limits.items()):
            if len(key) == 1:
                key = key[0]
    ...
    def parent_child_map(self, gff_handle):
        ...
        for parent_id, parent_type in list(parent_sts.items()):
            for child_type in child_sts[parent_id]:
                pc_map[parent_type].append(child_type)
        pc_final_map = dict()
        for ptype, ctypes in list(pc_map.items()):
